export const NextImageMock = ({ objectFit, objectPosition, ...props }) => {
    ...
    if (width !== undefined && height !== undefined && props.layout !== 'fill') {
      if (props.layout === 'responsive') {
        ...
        sizerSvg = `<svg width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg" />`
    ...
    // Add next/image mock without static warning
    // else {
    //     throw new Error(
    ...
    return (
      <div style={wrapperStyle}>
        ...
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img {...props} decoding="async" alt={'mock image'} style={imgStyle} />
      </div>
    )
  }
